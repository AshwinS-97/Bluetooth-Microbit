...
float estimate_heading_z(const float* accData, const float* magData, float* angles) {
...
float estimate_heading_y(const float* accData, const float* magData, float* angles) {
    // Signs choosen so that, when axis is down, the value is + 1g
  float accl_x = -accData[2];
  float accl_y = accData[0];
  float accl_z = accData[1];

  // Signs should be choosen so that, when the axis is down, the value is + positive.
  // But that doesn't seem to work ?...
  float magn_x = magData[2];
  float magn_y = -magData[0];
  float magn_z = -magData[1];  
...
void estimate_angles(const float* accData, const float* magData, float* angles) {;
        // print_float(angles[0] ,0);
        // printf("\t");
        // print_float(angles[1] ,0);
        // printf("\t");
        // print_float(angles[2] ,0);
        // printf("\n\r");
    /**
